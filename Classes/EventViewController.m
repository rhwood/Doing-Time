    ...
    if (showPercentage || showTotals) {
        ...
        if (!inPast) {
            ...
            if (left == 0) {
                ...
                _daysLeft.text = [NSString localizedStringWithFormat:NSLocalizedString(@"%ld %@ left", @"The number (%d) of days (%@) remaining"), left, days];
            } else if (!showTotals && showPercentage) {
                ...
                _daysLeft.text = [NSString localizedStringWithFormat:NSLocalizedString(@"%ld %@ (%2.4g%%) left", @"The number (%d) of days (%@) remaining with the percentage remaining in parenthesis"), left, days, interval * left * 100];
            }
        } else if (showTotals) {
